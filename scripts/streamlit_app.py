...
def query_influxdb(flux_query: str):
    ...
    Tối ưu hóa: Sử dụng @st.cache_data với TTL ngắn (30 giây) để tránh việc 
    truy vấn dồn dập vào Database khi Dashboard tự động làm mới, 
    giúp giảm tải Network I/O và CPU Handshake.
    """
    try:
        with InfluxDBClient(url=INFLUX_URL, token=INFLUX_TOKEN, org=INFLUX_ORG) as client:
            return client.query_api().query_data_frame(flux_query)
...
if tab_selection == "Dữ liệu Thời tiết Live":
    ...
    if not df_live.empty:
        ...
        c1.metric("Nhiệt độ", f"{live_data['temperature']} °C")
        c2.metric("Tốc độ gió", f"{live_data['windspeed']} km/h")
        c3.metric("Hướng gió", get_wind_direction(live_data['winddirection']))

        # B. Hybrid Logic: So sánh với Gold Layer
        st.divider()
        st.subheader("🔄 Phân tích Hybrid (So sánh Lịch sử)")
    ...
    st.subheader("📈 Xu hướng 30 ngày qua (Granular)")
    ...
    if not df_30d.empty:
        st.line_chart(df_30d.set_index('_time')['_value'])
...
# -----------------------------------------------------------------------------
    ...
    # A. Real-time AQI
    flux_aqi = f'from(bucket: "{INFLUX_BUCKET}") |> range(start: -1h) |> filter(fn: (r) => r._measurement == "aqi_metrics" and r._field == "aqi") |> last()'
    ...
    if not df_aqi.empty:
        ...
        if not df_gold_aqi.empty:
            ...
            st.metric("Chênh lệch với trung bình lịch sử", f"{int(val_aqi)}", f"{val_aqi - avg_h:+.1f} AQI", delta_color="inverse")
    ...
    st.subheader("📊 Diễn biến AQI trong 30 ngày gần nhất")
    ...
    if not df_aqi_30.empty:
        st.area_chart(df_aqi_30.set_index('_time')['_value'])
...
# -----------------------------------------------------------------------------
    ...
    if not df_summary.empty:
        ...
        m1.metric("Nhiệt độ đỉnh lịch sử", f"{df_summary['record_temp'].values[0]} °C")
    ...
    st.subheader("📋 Bảng tổng hợp thời tiết theo ngày (Gần đây)")
    weather_table_query = """
    ...
    st.dataframe(df_weather_table, use_container_width=True)
